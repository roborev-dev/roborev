...
func TestHandleEnqueueBackupAgentCanonicalization(t *testing.T) {
	server, db, tmpDir := newTestServer(t)
	...
	t.Run("unknown backup agent is cleared", func(t *testing.T) {
		...
		if job.BackupAgent != "" {
			t.Errorf("expected backup_agent cleared for unknown agent, got %q", job.BackupAgent)
		}
	})
	...
	t.Run("backup same as primary is cleared", func(t *testing.T) {
		...
		if job.BackupAgent != "" {
			t.Errorf("expected backup_agent cleared when same as primary, got %q", job.BackupAgent)
		}
	})
}
