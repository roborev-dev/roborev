...
func TestInstallClaudeWhenDirExists(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(skillsDir, "roborev-design", "SKILL.md")); err != nil {
		t.Error("expected roborev-design/SKILL.md to exist")
	}
...
func TestUpdateOnlyUpdatesInstalled(t *testing.T) {
	...
	t.Run("updates Claude with respond skill only", func(t *testing.T) {
		...
		// Should update respond (existed) and install address+design+fix (didn't exist)
		if len(results) > 0 {
			if len(results[0].Updated) != 1 {
				t.Errorf("expected 1 updated (respond), got %d", len(results[0].Updated))
			}
			if len(results[0].Installed) != 3 {
				t.Errorf("expected 3 installed (address, design, fix), got %d", len(results[0].Installed))
			}
		}
	})
