...
func (s *commentAPIServer) handler(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == http.MethodGet && r.URL.Path == "/repos/owner/repo/pulls/17/reviews":
		require.NoError(s.t, json.NewEncoder(w).Encode(s.reviewsByPR[17]))
	case r.Method == http.MethodGet && r.URL.Path == "/repos/owner/repo/pulls/17/comments":
		require.NoError(s.t, json.NewEncoder(w).Encode(s.inlineCommentsByPR[17]))
	case r.Method == http.MethodGet && r.URL.Path == "/repos/owner/repo/collaborators":
		require.NoError(s.t, json.NewEncoder(w).Encode(s.collaborators))
	default:
		http.NotFound(w, r)
	}
}
...
func (s *commentAPIServer) writeIssueComments(w http.ResponseWriter, prNumber int) {
	...
	require.NoError(s.t, json.NewEncoder(w).Encode(s.issueCommentsByPR[prNumber]))
}
...
func (s *commentAPIServer) captureCreate(w http.ResponseWriter, r *http.Request) {
	...
	require.NoError(s.t, json.NewEncoder(w).Encode(&googlegithub.IssueComment{
		ID:   ptr(int64(999)),
...
func TestCloneURL(t *testing.T) {
	...
	authenticated, err := CloneURL("owner/repo", "abc123")
	require.NoError(t, err)
	assert.Equal(t, "https://x-access-token:abc123@github.com/owner/repo.git", authenticated)
}
