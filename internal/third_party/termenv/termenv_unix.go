...
func (o Output) termStatusReport(sequence int) (string, error) {
	...
	// roborev patch: tmux sessions often expose TERM=xterm-256color so the
	// TERM check above does not fire, yet probing for OSC 11 / cursor
	// position can leak escape sequences into the parent shell when the
	// tmux client doesn't consume them. Whenever $TMUX is set we are
	// inside a tmux session — skip the probe entirely.
	// See internal/third_party/termenv/PATCHES.md for context.
	if o.environ.Getenv("TMUX") != "" {
		return "", ErrStatusReport
	}
