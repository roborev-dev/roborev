...
func (p *PgPool) UpsertJob(ctx context.Context, j SyncableJob, pgRepoID int64, pgCommitID *int64) error {
	_, err := p.pool.Exec(ctx, `
		...
		ON CONFLICT (uuid) DO UPDATE SET
			...
			provider = COALESCE(EXCLUDED.provider, review_jobs.provider),
			requested_model = COALESCE(EXCLUDED.requested_model, review_jobs.requested_model),
			requested_provider = COALESCE(EXCLUDED.requested_provider, review_jobs.requested_provider),
