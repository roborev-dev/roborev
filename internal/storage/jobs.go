...
// another producer won the race (no-op).
...
// design rows.
//
// The query is the exact complement of the partial unique index, so a
// `true` return guarantees an `INSERT ... ON CONFLICT DO NOTHING` would
// be a no-op. This is a performance shortcut; correctness is enforced by
