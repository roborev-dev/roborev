...
func TestHasSeverityAtOrAbove(t *testing.T) {
	tests := []struct {
		...
		{
			"high filter finds structured high",
			"- High — SQL injection in query.go:42",
			"high", true,
		},
		{
			"high filter finds critical",
			"- Critical — RCE via eval",
			"high", true,
		},
		{
		...
		{
			"legend is not a finding",
			"Severity levels:\nHigh - immediate action required.\nLow - minor issues.",
			"high", false,
		},
		{
			"high-level overview is not a finding",
		...
		{
			"low filter matches low finding",
			"- Low — minor naming issue",
			"low", true,
		},
		{
			"unrecognized filter returns true",
			"No issues found.",
			"bogus", true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := HasSeverityAtOrAbove(
				tt.output, tt.minSeverity,
			)
			if got != tt.want {
				t.Errorf(
					"HasSeverityAtOrAbove(%q, %q) = %v, want %v",
