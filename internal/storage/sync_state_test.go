...
func TestGetMachineID_EmptyValueRegeneration(t *testing.T) {
	...
	err := db.SetSyncState(SyncStateMachineID, "")
	if err != nil {
	...
	stored, err := db.GetSyncState(SyncStateMachineID)
	if err != nil {
...
func TestSyncWorker(t *testing.T) {
	...
	t.Run("SyncNowReturnsErrorWhenNotRunning", func(t *testing.T) {
		db := openTestDB(t)
		defer db.Close()
	...
	t.Run("FinalPushReturnsNilWhenNotConnected", func(t *testing.T) {
		db := openTestDB(t)
		defer db.Close()
