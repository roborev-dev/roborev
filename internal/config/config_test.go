...
func TestHasFindingsAtOrAbove(t *testing.T) {
	tests := []struct {
		...
		{
			"empty filter always returns true",
			"- Severity: low - minor naming issue",
			"", true,
		},
		{
			"low filter always returns true",
			"- Severity: low - minor naming issue",
			"low", true,
		},
		{
			"high filter finds high",
			"- **Severity**: high\n- SQL injection in query.go:42",
			"high", true,
		},
		{
			"high filter finds critical",
			"- Severity: critical - RCE via eval",
			"high", true,
		},
		{
			"high filter misses low-only",
			"- Severity: low - missing docstring",
			"high", false,
		},
		{
			"medium filter finds medium",
			"Found issue (medium) in handler.go",
			"medium", true,
		},
		{
			"critical filter misses high",
			"- Severity: high - buffer overflow",
			"critical", false,
		},
		{
			"case insensitive",
			"- Severity: HIGH - important bug",
			"high", true,
		},
		{
			"word boundary: highly does not match high",
			"The code is highly readable. Severity: low.",
			"high", false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := HasFindingsAtOrAbove(tt.output, tt.minSeverity)
			if got != tt.want {
				t.Errorf(
					"HasFindingsAtOrAbove(%q, %q) = %v, want %v",
					tt.output, tt.minSeverity, got, tt.want,
				)
			}
		})
	}
