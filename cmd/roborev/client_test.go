...
// checkClientTestError is a helper to verify error messages
func checkClientTestError(t *testing.T, err error, want string) {
	t.Helper()
	if want != "" {
		if err == nil {
...
func TestGetCommentsForJob(t *testing.T) {
	t.Run("returns responses for job", func(t *testing.T) {
		...
		md := daemonFromHandler(t, http.HandlerFunc(handler))

		responses, err := getCommentsForJob(md.Server.URL, 42)
		checkClientTestError(t, err, "")
		if len(responses) != 2 {
	...
	t.Run("returns error on non-200", func(t *testing.T) {
		...
		md := daemonFromHandler(t, handler)

		_, err := getCommentsForJob(md.Server.URL, 42)
		if err == nil {
...
func TestWaitForReview(t *testing.T) {
	t.Run("returns review when job completes", func(t *testing.T) {
		...
		review, err := waitForReview(md.Server.URL, 1)
		checkClientTestError(t, err, "")
	...
	t.Run("polls until job transitions from queued to done", func(t *testing.T) {
		...
		review, err := waitForReviewWithInterval(md.Server.URL, 1, 1*time.Millisecond)
		checkClientTestError(t, err, "")
	...
	t.Run("returns error on job failure", func(t *testing.T) {
		...
		md := daemonFromHandler(t, newMockHandler(t, "GET", "/api/jobs", resp, 0))

		_, err := waitForReview(md.Server.URL, 1)
		checkClientTestError(t, err, "agent crashed")
	})
	...
	t.Run("returns error on job canceled", func(t *testing.T) {
		...
		_, err := waitForReview(md.Server.URL, 1)
		checkClientTestError(t, err, "canceled")
	})
}
...
func TestFindJobForCommit(t *testing.T) {
	tests := []struct {
		name         string
		buildHandler func(t *testing.T, repoDir string) http.HandlerFunc
		commitSHA    string
		expectedID   int64
		expectFound  bool
		expectError  string
	}{
		{
			...
			commitSHA: "def456",
			buildHandler: func(t *testing.T, repoDir string) http.HandlerFunc {
				return mockSequenceHandler(t, MockStep{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := tt.buildHandler(t, repoDir)

			md := daemonFromHandler(t, handler)

			job, err := findJobForCommit(md.Server.URL, repoDir, tt.commitSHA)

			checkClientTestError(t, err, tt.expectError)
			if tt.expectError != "" {
				return
			}
