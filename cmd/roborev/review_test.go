...
func setupRepoWithCommits(
	t *testing.T,
) (*TestGitRepo, *http.ServeMux, string, string) {
	repo, mux := setupTestEnvironment(t)
	firstSHA := repo.CommitFile("file1.txt", "first", "first commit")
	secondSHA := repo.CommitFile("file2.txt", "second", "second commit")
	return repo, mux, firstSHA, secondSHA
}
...
func handleJobsDone(w http.ResponseWriter, r *http.Request) {
...
func mockWaitableReview(
	...
	t.Helper()
	mux.HandleFunc("/api/enqueue", mockEnqueueQueued)
	mux.HandleFunc("/api/jobs", handleJobsDone)
...
func handleSkippedEnqueue(w http.ResponseWriter, r *http.Request) {
	respondJSON(w, http.StatusOK, map[string]any{
		"skipped": true,
...
func mockSkippedEnqueue(t *testing.T, mux *http.ServeMux) {
	mux.HandleFunc("/api/enqueue", handleSkippedEnqueue)
}

func mockEnqueueQueued(w http.ResponseWriter, r *http.Request) {
	job := storage.ReviewJob{ID: 1, GitRef: "abc123", Agent: "test", Status: "queued"}
	respondJSON(w, http.StatusCreated, job)
}

func mockReviewSuccess(w http.ResponseWriter, r *http.Request) {
	respondJSON(w, http.StatusOK, storage.Review{
		ID:     1,
		JobID:  1,
		Agent:  "test",
...
func TestEnqueueCmdPositionalArg(t *testing.T) {
	tests := []struct {
		...
		{"positional arg overrides default HEAD", []string{"{SHA}"}, "{SHA}"},
		{"sha flag works", []string{"--sha", "{SHA}"}, "{SHA}"},
		{"defaults to HEAD", []string{}, "HEAD"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo, mux, firstSHA, _ := setupRepoWithCommits(t)
			reqCh := mockEnqueue(t, mux)

			shortFirstSHA := firstSHA[:7]

			args := []string{"--repo", repo.Dir}
			for _, arg := range tt.args {
				args = append(args, strings.ReplaceAll(arg, "{SHA}", shortFirstSHA))
			}

			_, _, err := executeReviewCmd(args...)
			if err != nil {
				t.Fatalf("enqueue failed: %v", err)
			}

			req := <-reqCh
			wantRef := strings.ReplaceAll(tt.wantRef, "{SHA}", shortFirstSHA)

			if req.GitRef != wantRef {
				t.Errorf("Expected SHA %s, got %s", wantRef, req.GitRef)
			}
			if tt.wantRef != "HEAD" && req.GitRef == "HEAD" {
				t.Error("Received HEAD instead of expected arg - bug not fixed!")
			}
		})
	}
}
...
func TestEnqueueSkippedBranch(t *testing.T) {
	tests := []struct {
		...
		{"prints message", []string{}, "Skipped"},
		{"quiet mode suppresses output", []string{"--quiet"}, ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo, mux, _, _ := setupRepoWithCommits(t)
			mockSkippedEnqueue(t, mux)

			args := append([]string{"--repo", repo.Dir}, tt.args...)
			stdout, _, err := executeReviewCmd(args...)
			...
			if tt.wantStdout != "" {
				if !strings.Contains(stdout, tt.wantStdout) {
					t.Errorf("expected stdout to contain %q, got: %q", tt.wantStdout, stdout)
				}
			} else {
				if stdout != "" {
...
func TestWaitQuietVerdictExitCode(t *testing.T) {
	...
	tests := []struct {
		...
		{"passing review exits 0 with no output", "No issues found.", false},
		{"failing review exits 1 with no output", "Found 2 issues:\n1. Bug in foo.go\n2. Missing error handling", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo, mux, _, _ := setupRepoWithCommits(t)
			mockWaitableReview(t, mux, tt.output)
...
func TestWaitForJobUnknownStatus(t *testing.T) {
	...
	t.Run("unknown status exceeds max retries", func(t *testing.T) {
		...
		poller := &mockJobPoller{
			statuses: []string{
				"future_status", "future_status", "future_status", "future_status", "future_status",
				"future_status", "future_status", "future_status", "future_status", "future_status",
				"future_status",
			},
		}
		...
		if poller.callCount != 10 {
			t.Errorf("expected 10 retries, got %d", poller.callCount)
		}
	})
	...
	t.Run("counter resets on known status", func(t *testing.T) {
		...
		mux.HandleFunc("/api/enqueue", mockEnqueueQueued)
		mux.HandleFunc("/api/jobs", poller.HandleJobs)
		mux.HandleFunc("/api/review", mockReviewSuccess)
...
func TestReviewSinceFlag(t *testing.T) {
	...
	t.Run("since with invalid ref fails", func(t *testing.T) {
		repo, _, _, _ := setupRepoWithCommits(t)
