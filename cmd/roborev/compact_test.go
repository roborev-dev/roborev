...
func TestIsValidConsolidatedReview(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "invalid_failed_to",
			output: "Failed to analyze the codebase",
			want:   false,
		},
		{
			name:   "invalid_cannot",
			output: "Cannot find the specified file",
			want:   false,
		},
		{
			name:   "invalid_unable_to",
			output: "Unable to complete the review",
			want:   false,
		},
		{
			name:   "valid_with_severity",
			output: "High severity issue found in authentication",
			want:   true,
		},
		{
			name:   "valid_with_critical",
			output: "Critical: Buffer overflow detected",
			want:   true,
		},
		{
			name:   "valid_with_medium",
			output: "Medium priority: improve error handling",
			want:   true,
		},
		{
			name:   "valid_with_low",
			output: "Low: consider adding documentation",
			want:   true,
		},
		{
			name:   "valid_with_go_file_reference",
			output: "Issue in main.go:123 needs attention",
			want:   true,
		},
		{
			name:   "valid_with_py_file_reference",
			output: "Problem found in script.py:45",
			want:   true,
		},
		{
